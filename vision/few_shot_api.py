...
def load_fewshot_example_images(examples_dir="C:/Users/LenovoGamerNotebook/Desktop/TDK/archive/few_shot_examples", max_per_class=5):
            ...
            if label_name.lower() in filename.lower():
                try:
                    with Image.open(filepath) as img:
                        if len(example_images[label_name]) < max_per_class:
                            example_images[label_name].append(image_to_base64(img))
                except Exception as e:
                    print(f"Error loading image {filepath}: {e}")
                break  # Skip remaining checks on this filename
    return example_images
...
def analyze_image_fewshot(image_path, example_images, labels, label_name_to_id):
    try:
    ...
    parts = [
        {"text": "Analyze this medical image and select the most suitable label from the following options.\n"
                 "If you are unsure, choose the label you are most confident in.\n"
                 "Respond with only the number and label (e.g., '2: CXR').\n\n"
                 "Here are examples per class:\n"}
    ]

    for label_name, img_list in example_images.items():
        class_id = label_name_to_id[label_name]
        class_label = labels[class_id]
        parts.append({"text": f"- Examples for {class_label}:\n"})
...
def save_accuracy_chart(class_stats, overall_accuracy, filename='accuracy_report.pdf'):
    ...
    bars = plt.bar(classes, accuracies, color='purple')
    plt.ylim(70, 100)
    plt.ylabel('Accuracy (%)')
...
example_images = load_fewshot_example_images(max_per_class=3)
if not example_images:
    print("ERROR: No example images found in few-shot examples directory.")
    exit(1)
...
for i, image_path in enumerate(image_files, 1):
    ...
    prediction_text = analyze_image_fewshot(image_path, example_images, labels, label_name_to_id)
    pred_label_id, pred_label_name = parse_prediction(prediction_text)

    # --- FIRST, determine if the prediction is correct
    is_correct = False
    ...
    # --- NOW, print everything, including whether it's correct
